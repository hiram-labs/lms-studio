...
To enable an organization's headsets to run training modules, a small application must be installed. This application facilitates module management and authentication on the headsets.

---

### **Steps for Headset Onboarding**

1. **Navigate to the Headset Onboarding Page**

   - Access the **Headset Onboarding Page** (see useful links) for detailed instructions on installation.
   - Ensure that the organization's headset is prepared for the onboarding process.

2. **Install the Headset Application**
   - Follow the step-by-step guide provided on the Headset Onboarding Page to install the application on the headset.
...
3. **Provide the Required Trainee Details**
   ...
   - **PIN:** Enter the trainee’s PIN.
     > **Important:** PINs must be unique across the platform. Leave empty for an auto generated PIN.
...
4. **Complete User Information**
   ...
   - Fill in the required fields for the user:
     ...
     - **Email**
     - **Token**
   - Ensure the `Role` field is set to **STUDIO_BASIC_USER_ROLE**.
